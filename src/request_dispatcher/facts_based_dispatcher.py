...
class BasicRequestDispatcherHandler(RequestMessageHandler):
    ...
    def handle_request(self, request: Request) -> str:
        ...
        last_request_status_src = splitted_status[0]
        ...
        #fake logic
        LOGGER.info(f'last_request_status_src {last_request_status_src}, we are gonna move according to the fst_config')
        if last_request_status_src in self.fst_config:
            LOGGER.info(f'last_request_status_src {last_request_status_src} dispatch it to {self.fst_config[last_request_status_src]["next"]}')
            return self.fst_config[last_request_status_src]['queue']
        else:
            LOGGER.info(f'last_request_status_src {last_request_status_src} dispatch it to response queue')
